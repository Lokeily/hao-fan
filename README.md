...
「好翻」是一款浏览器翻译扩展：保留网页原文，把译文自然地显示在原文下方，逐段对照阅读。它把请求直接发往你自己的翻译服务商，不依赖任何中转服务器，也不上传你的浏览内容。

- 免费开源，无使用额度，无遥测、无广告追踪
- 内置 12 家 AI 服务商（含任意 OpenAI 兼容自定义接口）与 3 家传统机器翻译，也支持本地模型（Ollama）
- API Key 只保存在本地浏览器，翻译请求直连所选服务商

## 工作原理

为保证「一一对应」，以下每一点都对应仓库中的具体实现：

- **双层架构**：内容脚本（`entrypoints/content.ts`）负责页面注入与渲染；后台 Service Worker（`entrypoints/background.ts`）负责密钥管理与翻译 API 调用，二者通过 `runtime` 消息与持久端口通信，契合 Manifest V3 的生命周期约束。
- **流式回填**：长文本经路由后走 SSE 长连接，首段边生成边显示，逐字回流到对应节点，无需等待整段完成。
- **注入防护**：网页标题与前文等不可信内容，被显式降级为「数据而非指令」追加到用户消息末尾（而非系统提示词），降低恶意页面操纵译文的风险；系统提示词前缀因此保持稳定，利于供应商提示词缓存。
- **译后自检**：译文生成后校验原文中的数字 / URL / 邮箱 / 占位符 / 代码 token 是否完整保留，缺失则自动重试校正并在界面标记。
- **省 Token 策略**：句子级缓存、重复文本合并、术语库命中零请求、逐段语言检测（已是目标语言即本地跳过，杜绝「中译中」）、术语注入上限可调，多层叠加降低调用成本。
- **代码 / 库名保护**：翻译前把代码标识符与品牌专名（useState、react-dom、React、Vue 等）替换为占位符、译后还原，库名不会被翻译成中文；占位符比译文短，顺带省 completion Token。
- **稳健渲染**：MutationObserver 主动排除自身注入节点以防递归；译文落点统一使用 `textContent` 防止 XSS；后台断连时回退首段以避免空白。
...
| 流式输出 | 基于 SSE 长连接逐字回流，首段无需等待整段完成即显示 |
| 上下文感知翻译 | 结合页面标题与前段译文，长文代词指代与术语更连贯 |
| 翻译质量自检 | 译后校验数字 / URL / 邮箱 / 占位符 / 代码 token 是否完整保留，缺失自动重试校正并标记提示 |
| 可见区域优先 | 先翻译当前屏幕内容，滚动时继续翻译新进入视口的内容 |
| 动态内容翻译 | 弹窗、菜单、无限滚动等异步加载的内容自动补译 |
| 划词翻译 | 选中文字即译，结果展示在独立浮层，不改写网页正文 |
| 智能语言检测 | 按段落检测语种，已是目标语言即本地跳过（0 Token），杜绝「中译中」 |
| 代码 / 库名保护 | useState、react-dom、React 等代码标识符与品牌专名翻译前后原样保留 |
| 手动翻译模式 | 自动整页与手动点击 / 划词两种模式可切换，最省 Token |
| 译文可编辑 · 术语自学习 | hover 译文即可修改，并自动抽取术语沉淀进个人术语表 |
| 图片翻译 | 网页图片与本地图片 OCR + 翻译（需支持视觉的模型） |
| 按网站暂停 | 一键暂停当前网站、清理译文，无需刷新即可恢复 |
| 多引擎路由 | 备用引擎故障转移（限流 / 报错自动切换）+ 长文强模型路由 |
| 节省 Token | 句子级缓存、重复文本合并、术语库命中零请求、逐段语言检测跳过、标识符占位符保护、术语注入上限可调 |
...
- **AI 服务（12 家）**：DeepSeek、OpenAI、Google Gemini、OpenRouter、智谱 GLM、腾讯混元、通义千问、Kimi、百川智能、豆包、Ollama 本地模型，以及任意 OpenAI 兼容自定义接口
...
追求最省 Token？在设置中把「翻译模式」切到「手动点击 / 划词」：整页不自动翻译，点击段落或划选文字才翻译，Token 开销最小。
...
- 本地翻译缓存默认 30 天有效期与容量上限（LRU 2000 条），可在设置中关闭
